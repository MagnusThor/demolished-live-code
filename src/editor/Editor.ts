...
import {
    ...
    indentOnInput,
    syntaxHighlighting
} from '@codemirror/language'; // NOT @codemirror/highlight
import { EditorState } from '@codemirror/state';
import { EditorView, keymap } from '@codemirror/view';


import { defaultComputeShader } from './wgsl/defaultComputeShader';
...
import { WGSLShaderRenderer, initWebGPU } from '../engine/renderer/webgpu/wgslShaderRenderer';
...
export class Editor {
    ...
    async setupEditor(shader: StoredShader) {
        ...
        const state = EditorState.create({
            ...
            extensions: [
                ...
                basicSetup, rust(),
                keymap.of([
                    ...(defaultKeymap as any), ...customKeyMap, indentWithTab
                ]),
    ...
    setupUI(): void {
        ...
        DOMUtils.on<HTMLElement>("click", "#copy-editor-link", (evt, el) => {
            const url = `${window.location.origin}${location.pathname}preview/?shader=${this.currentShader.id}`;
        ...
        DOMUtils.get<HTMLButtonElement>("#btn-run-shader").addEventListener("click", async (e) => {
            ...
            if (this.isRunning) {
                ...
                connectCanvasToVideo(800, 450, 60)
                DOMUtils.get("#video-result").classList.remove("d-none");
                DOMUtils.get("#result-canvas").classList.add("d-none");
            }
